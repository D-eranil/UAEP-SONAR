...
public partial class bookavenue : System.Web.UI.Page
{
    private static string[] formats = new string[]
   {
    ...
    private void SendContactUsFormDetail()
    {
        ...
        list.Add("$Fname$");
        values.Add(Request["Fname"]);

        list.Add("$Lname$");
        values.Add(Request["Lname"]);

        list.Add("$Cname$");
        values.Add(Request["Cname"]);

        list.Add("$BookingTypeOption$");
        if (!String.IsNullOrEmpty(Request["BookingTypeOption"]))
            values.Add(Request["BookingTypeOption"]);
        else
        ...
        values.Add("Umm Al Emarat Park: Book A Venue");
        ...
        string subject = "Book A Venue";
        string templateFile = "/email-templates/email_template_BookaVenueAdmin.html";
        ...
        string customTableClassName = "BookAVenue.Form";
        DataClassInfo customTable = DataClassInfoProvider.GetDataClassInfo(customTableClassName);
        if (customTable != null)
        {
            CustomTableItem newCustomTableItem = CustomTableItem.New(customTableClassName);

            newCustomTableItem.SetValue("FirstName", StripHTML(Request["Fname"]));
            newCustomTableItem.SetValue("LastName", StripHTML(Request["Lname"]));
            newCustomTableItem.SetValue("CompanyName", StripHTML(Request["Cname"]));
            newCustomTableItem.SetValue("MobileNumber", "+" + Request["telCode"] + " " + Request["number"]);
            newCustomTableItem.SetValue("Email", Request["email"]);
            newCustomTableItem.SetValue("EventDate", StripHTML(Request["date"]));
            newCustomTableItem.SetValue("Time", StripHTML(Request["time"]));
            newCustomTableItem.SetValue("EventType", StripHTML(Request["type"]));
            newCustomTableItem.SetValue("VenueOfInterest", Request["question"]);
            newCustomTableItem.SetValue("BookingTypeOption", Request["BookingTypeOption"]);
            newCustomTableItem.SetValue("Description", StripHTML(Request["message"]));
    ...
    {
        string templateFile = "/email-templates/email_template_BookAVenue.html";
        string logopath = null;
        ...
        email = email.Replace("$MainHeading$", "Umm Al Emarat Park: Book A Venue");
        email = email.Replace("$Fname$", Request["Fname"]);
        email = email.Replace("$Lname$", Request["Lname"]);
        logopath = "http://" + Request.Url.DnsSafeHost + "/email-templates/img/info-header.jpg";
        email = email.Replace("$logoUrl$", logopath);
        sr.Close();
        sr.Dispose();

        EmailSender.SendEmail(PropertyReader.ReadProperty("fromEmail"), userEmail, PropertyReader.ReadProperty("ccEmail"), "We got it! – RE: Request to book a venue", email, "");
    }
